...
$$.Graph = {"":
 ["edges", "degree", "numVertices", "numEdges", "directed"],
 "super": "Object",
 insertEdge$3: function(x, y, directed) {
  var p = $.EdgeNode$(y, 0);
  var t1 = this.edges;
  if (x !== (x | 0))
    throw $.iae(x);
  ...
  var t3 = t2[x];
  if (typeof t3 !== 'number')
    return this.insertEdge$3$bailout(1, x, y, directed, t3, t2);
  t2[x] = t3 + 1;
  if (!directed)
    this.insertEdge$3(y, x, true);
  else {
    t1 = this.numEdges;
    if (typeof t1 !== 'number')
      return this.insertEdge$3$bailout(2, t1, 0, 0, 0, 0);
    this.numEdges = t1 + 1;
  }
},
 insertEdge$3$bailout: function(state, env0, env1, env2, env3, env4) {
  switch (state) {
    case 1:
      var x = env0;
      var y = env1;
    ...
    case 2:
      if (state === 0 && !directed)
        ...
        switch (state) {
          ...
          case 2:
            state = 0;
            this.numEdges = $.add(t1, 1);
        }
  }
},
 toString$0: function() {
  var output = $.StringBuffer_StringBuffer('');
  for (var i = 1; $.leB(i, this.numVertices); ++i) {
    $.add$1(output, i === 1 ? '' : '\n');
    $.add$1(output, $.S(i) + ':');
    var t1 = this.edges;
    if (i >= t1.length)
    ...
    var p = t1[i];
    for (; !(p == null);) {
      $.add$1(output, ' ' + $.S(p.get$y()));
      p = p.get$next();
    }
  }
  return $.toString(output);
},
 Graph$1: function(isDirected) {
  this.degree = $.ListImplementation_List(1001);
  this.edges = $.ListImplementation_List(1001);
...
$$.Circle = {"":
 ...
 "super": "Vec2",
 toString$0: function() {
  return 'X:(' + $.S($.toStringAsFixed(this.x, 3)) + ', Y:' + $.S($.toStringAsFixed(this.y, 3)) + ', R:' + $.S(this.radius) + ')';
}
};
...
$.Graph_Graph$fromSimpleText = function(graphText) {
  var g = $.Graph$(false);
  var edgeInfo = $.split(graphText, '\n');
  g.numVertices = $.parseInt($.index(edgeInfo, 0));
  $.removeLast(edgeInfo);
  $.removeAt$1(edgeInfo, 0);
  for (var t1 = $.iterator(edgeInfo); t1.hasNext$0() === true;) {
    var nodeInfo = $.split(t1.next$0(), ',');
    g.insertEdge$3($.parseInt($.index(nodeInfo, 0)), $.parseInt($.index(nodeInfo, 1)), false);
  }
  return g;
};
...
$.onSuccess = function(req) {
  $.aGraph = $.Graph_Graph$fromSimpleText(req.get$responseText());
  $.print($.Circle$(0, 0, 10));
  $.print($.aGraph);
};
...
$.main = function() {
  ...
  $.HttpRequest_HttpRequest$get('web/data/simpleGraph00.txt', $.onSuccess);
};
...
$.EdgeNode$ = function(pY, pWeight) {
  return new $.EdgeNode(pY, pWeight, null);
};
...
$.Graph$ = function(isDirected) {
  var t1 = new $.Graph(null, null, 0, 0, isDirected);
  t1.Graph$1(isDirected);
  return t1;
};
...
$.Circle$ = function(pX, pY, radius) {
  return new $.Circle(radius, 6.283185307179586, pX, pY);
};
...
$.$defineNativeClass('DOMWindow', ["length?", "status?"], {
...
//@ sourceMappingURL=GraphApp.dart.js.map